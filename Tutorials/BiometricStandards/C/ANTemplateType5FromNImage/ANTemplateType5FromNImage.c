	...
	)
{
	...
	result = ANTemplateAddType5RecordFromNImage(hANTemplate, isr, ca, hlrBinImage, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANTemplateAddType5RecordFromNImage, error code: %d"), result);
		goto FINALLY;
	}
