...
const NChar description[] = N_T("Demonstrates creation of ANTemplate with current version and type 13 record in it");
	...
	)
{
	...
	HNString hSrc = NULL;
	NResult result;
	BdifFPImpressionType imp = bfpitLatentImpression; // impression type
	...
	/*
		...
		Image must be compressed using valid compression algorithm for Type-13 record.
	...
	*/
	...
	// Create Type 13 record and add record to ANTemplate object
	result = ANTemplateAddType13RecordFromImageDataN(hANTemplate, imp, hSrc, hImageBuffer, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANTemplateAddType13RecordFromImageDataN, error code: %d"), result);
		goto FINALLY;
	}
...
int main(int argc, NChar **argv)
{
	...
	result = ANTemplateAddRecordType13(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
