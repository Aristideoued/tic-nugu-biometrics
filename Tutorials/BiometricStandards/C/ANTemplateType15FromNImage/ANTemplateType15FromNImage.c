	...
	)
{
	...
	/*
		...
		Image must be compressed using valid compression algorithm for Type-15 record.
	...
	*/
	...
	// Create Type 15 record and add record to ANTemplate object
	result = ANTemplateAddType15RecordFromImageDataN(hANTemplate, hSrc, hImageBuffer, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANTemplateAddType15RecordFromImageDataN, error code: %d"), result);
		goto FINALLY;
	}
	...
	if(NFailed(result))
	{
		printf(N_T("error in ANTemplateSaveToFileEx, error code: %d"), result);
		goto FINALLY;
	}
...
int main(int argc, NChar **argv)
{
	...
	const NChar * license = N_T("PalmClient");
	...
	result = ANTemplateAddRecordType15(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
