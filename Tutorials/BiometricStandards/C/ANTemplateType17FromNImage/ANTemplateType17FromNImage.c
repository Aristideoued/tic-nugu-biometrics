...
const NChar description[] = N_T("Demonstrates creation of ANTemplate with current version and type 17 record in it");
	...
	)
{
	...
	*/
	...
	// Create Type 17 record and add record to ANTemplate object
	result = ANTemplateAddType17RecordFromImageDataN(hANTemplate, hSrc, hImageBuffer, &hRecord);
	if (NFailed(result))
	{
		result = PrintErrorMsgWithLastError(N_T("error in ANTemplateAddType17RecordFromImageDataN, error code: %d"), result);
		goto FINALLY;
	}
...
int main(int argc, NChar **argv)
{
	...
	result = ANTemplateAddRecordType17(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7], argv[8]);
	if (NFailed(result))
	{
		goto FINALLY;
	}
