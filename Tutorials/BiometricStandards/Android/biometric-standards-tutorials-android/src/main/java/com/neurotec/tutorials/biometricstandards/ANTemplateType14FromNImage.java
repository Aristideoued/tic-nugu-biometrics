...
public final class ANTemplateType14FromNImage extends BaseActivity {
	...
	private Map<BDIFEncodingType, String> mANTemplateFileNames = new HashMap<BDIFEncodingType, String>() {{
		put(BDIFEncodingType.TRADITIONAL, "antemplate-type-14.dat");
		put(BDIFEncodingType.XML, "antemplate-type-14.xml");
	}};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSpinnerEncoding.setAdapter(new ArrayAdapter<BDIFEncodingType>(ANTemplateType14FromNImage.this, android.R.layout.simple_spinner_item, BDIFEncodingType.values()));
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try (ANTemplate template = new ANTemplate(ANTemplate.VERSION_CURRENT, mTot, mDai, mOri, mTcn, 0);
			 ...
			 ANType14Record ignored = template.getRecords().addType14(mSrc, imgBuffer)) {
			...
			requestToSaveFile(mANTemplateFileNames.get(mEncoding), new OnSelectionListener() {
				...
				public void onFileSelected(Uri path) {
					try {
						saveDataToUri(ANTemplateType14FromNImage.this, path, data);
						showMessage(getString(R.string.format_antemplate_saved_to, 14));
					} catch (IOException e) {
						showError(e);
					}
				}
	...
	final class InitializationTask extends AsyncTask<Object, Boolean, Boolean> {
		...
		protected Boolean doInBackground(Object... params) {
			try {
				List<String> obtainedLicenses = LicensingManager.getInstance().obtainLicenses(ANTemplateType14FromNImage.this, LICENSES);
