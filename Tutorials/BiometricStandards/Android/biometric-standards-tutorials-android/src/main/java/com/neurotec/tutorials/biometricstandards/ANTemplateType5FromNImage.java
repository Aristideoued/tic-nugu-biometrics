...
public final class ANTemplateType5FromNImage extends BaseActivity {
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try (ANTemplate template = new ANTemplate(ANTemplate.VERSION_40, mTot, mDai, mOri, mTcn, 0);
			 ...
			 ANType5Record ignored = template.getRecords().addType5(true, ANBinaryImageCompressionAlgorithm.NONE, image)) {
			...
			requestToSaveFile("antemplate-type-5.dat", new OnSelectionListener() {
				@Override
				public void onFileSelected(Uri path) {
					try {
						saveDataToUri(ANTemplateType5FromNImage.this, path, data);
						showMessage(getString(R.string.format_antemplate_saved_to, 5));
					} catch (IOException e) {
						showError(e);
					}
				}
	...
	final class InitializationTask extends AsyncTask<Object, Boolean, Boolean> {
		...
		protected Boolean doInBackground(Object... params) {
			try {
				List<String> obtainedLicenses = LicensingManager.getInstance().obtainLicenses(ANTemplateType5FromNImage.this, LICENSES);
