...
public final class IIRecordToNTemplate extends BaseActivity {
	...
	private void convert(Uri recordUri) throws IOException {
		...
		try (IIRecord iiRecord = new IIRecord(iiRecordData, BDIFStandard.ISO);
			...
			if (status == NBiometricStatus.OK) {
				...
				requestToSaveFile("ntemplate-from-iirecord.dat", new OnSelectionListener() {
					@Override
					public void onFileSelected(Uri path) {
						try {
							saveDataToUri(IIRecordToNTemplate.this, path, data);
	...
	final class InitializationTask extends AsyncTask<Object, Boolean, Boolean> {
		...
		protected Boolean doInBackground(Object... params) {
			try {
				List<String> obtainedLicenses = LicensingManager.getInstance().obtainLicenses(IIRecordToNTemplate.this, LICENSES);
