...
public final class ANTemplateType15FromNImage extends BaseActivity {

	private static final String TAG = ANTemplateType15FromNImage.class.getSimpleName();
	private static final int REQUEST_CODE_GET_IMAGE = 1;

	private static final String[] LICENSES = {"PalmClient"};

	private Map<BDIFEncodingType, String> mANTemplateFileNames = new HashMap<BDIFEncodingType, String>() {{
		put(BDIFEncodingType.TRADITIONAL, "antemplate-type-15.dat");
		put(BDIFEncodingType.XML, "antemplate-type-15.xml");
	}};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSpinnerEncoding.setAdapter(new ArrayAdapter<BDIFEncodingType>(ANTemplateType15FromNImage.this, android.R.layout.simple_spinner_item, BDIFEncodingType.values()));
	...
	private void convert(Uri imageUri) throws IOException {
		...
		// Create Type 15 record and add record to ANTemplate
		...
		try (ANTemplate template = new ANTemplate(ANTemplate.VERSION_CURRENT, mTot, mDai, mOri, mTcn, 0);
			 ...
			 ANType15Record ignored = template.getRecords().addType15(mSrc, imgBuffer)) {
			...
			requestToSaveFile(mANTemplateFileNames.get(mEncoding), new OnSelectionListener() {
				...
				public void onFileSelected(Uri path) {
					try {
						saveDataToUri(ANTemplateType15FromNImage.this, path, data);
						showMessage(getString(R.string.format_antemplate_saved_to, 15));
					} catch (IOException e) {
						showError(e);
					}
				}
	...
	final class InitializationTask extends AsyncTask<Object, Boolean, Boolean> {
		...
		protected Boolean doInBackground(Object... params) {
			try {
				List<String> obtainedLicenses = LicensingManager.getInstance().obtainLicenses(ANTemplateType15FromNImage.this, LICENSES);
