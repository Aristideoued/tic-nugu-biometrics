...
public final class ANTemplateType3FromNImage extends BaseActivity {
	...
	private void convert(Uri imageUri) throws IOException {
		...
		try (ANTemplate template = new ANTemplate(ANTemplate.VERSION_40, mTot, mDai, mOri, mTcn, 0);
			 ...
			 ANType3Record ignored = template.getRecords().addType3(true, imgBuffer)) {
			...
			requestToSaveFile("antemplate-type-3.dat", new OnSelectionListener() {
				@Override
				public void onFileSelected(Uri path) {
					try {
						saveDataToUri(ANTemplateType3FromNImage.this, path, data);
						showMessage(getString(R.string.format_antemplate_saved_to, 3));
					} catch (IOException e) {
						showError(e);
					}
				}
	...
	final class InitializationTask extends AsyncTask<Object, Boolean, Boolean> {
		...
		protected Boolean doInBackground(Object... params) {
			try {
				List<String> obtainedLicenses = LicensingManager.getInstance().obtainLicenses(ANTemplateType3FromNImage.this, LICENSES);
