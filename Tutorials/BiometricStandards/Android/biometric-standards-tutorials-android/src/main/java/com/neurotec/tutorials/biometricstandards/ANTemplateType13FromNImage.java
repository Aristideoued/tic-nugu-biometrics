...
public final class ANTemplateType13FromNImage extends BaseActivity {
	...
	private Map<BDIFEncodingType, String> mANTemplateFileNames = new HashMap<BDIFEncodingType, String>() {{
		put(BDIFEncodingType.TRADITIONAL, "antemplate-type-13.dat");
		put(BDIFEncodingType.XML, "antemplate-type-13.xml");
	}};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSpinnerEncoding.setAdapter(new ArrayAdapter<BDIFEncodingType>(ANTemplateType13FromNImage.this, android.R.layout.simple_spinner_item, BDIFEncodingType.values()));
	...
	private void convert(Uri imageUri) throws IOException {
		...
		// Create Type 13 record and add record to ANTemplate
		...
		try (ANTemplate template = new ANTemplate(ANTemplate.VERSION_CURRENT, mTot, mDai, mOri, mTcn, 0);
			 ...
			 ANType13Record ignored = template.getRecords().addType13(BDIFFPImpressionType.LATENT_IMPRESSION, mSrc, imgBuffer)) {
			...
			requestToSaveFile(mANTemplateFileNames.get(mEncoding), new OnSelectionListener() {
				...
				public void onFileSelected(Uri path) {
					try {
						saveDataToUri(ANTemplateType13FromNImage.this, path, data);
						showMessage(getString(R.string.format_antemplate_saved_to, 13));
					} catch (IOException e) {
						showError(e);
					}
				}
	...
	final class InitializationTask extends AsyncTask<Object, Boolean, Boolean> {
		...
		protected Boolean doInBackground(Object... params) {
			try {
				List<String> obtainedLicenses = LicensingManager.getInstance().obtainLicenses(ANTemplateType13FromNImage.this, LICENSES);
