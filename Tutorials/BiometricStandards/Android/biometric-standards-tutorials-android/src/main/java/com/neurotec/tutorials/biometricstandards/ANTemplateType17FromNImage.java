...
public final class ANTemplateType17FromNImage extends BaseActivity {
	...
	private Map<BDIFEncodingType, String> mANTemplateFileNames = new HashMap<BDIFEncodingType, String>() {{
		put(BDIFEncodingType.TRADITIONAL, "antemplate-type-17.dat");
		put(BDIFEncodingType.XML, "antemplate-type-17.xml");
	}};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSpinnerEncoding.setAdapter(new ArrayAdapter<BDIFEncodingType>(ANTemplateType17FromNImage.this, android.R.layout.simple_spinner_item, BDIFEncodingType.values()));
	...
	private void convert(Uri imageUri) throws IOException {
		...
		// Create Type 17 record and add record to ANTemplate
		...
		try (ANTemplate template = new ANTemplate(ANTemplate.VERSION_CURRENT, mTot, mDai, mOri, mTcn, 0);
			 ...
			 ANType17Record ignored = template.getRecords().addType17(mSrc, imgBuffer)) {
			...
			requestToSaveFile(mANTemplateFileNames.get(mEncoding), new OnSelectionListener() {
				...
				public void onFileSelected(Uri path) {
					try {
						saveDataToUri(ANTemplateType17FromNImage.this, path, data);
						showMessage(getString(R.string.format_antemplate_saved_to, 17));
					} catch (IOException e) {
						showError(e);
					}
				}
	...
	final class InitializationTask extends AsyncTask<Object, Boolean, Boolean> {
		...
		protected Boolean doInBackground(Object... params) {
			try {
				List<String> obtainedLicenses = LicensingManager.getInstance().obtainLicenses(ANTemplateType17FromNImage.this, LICENSES);
