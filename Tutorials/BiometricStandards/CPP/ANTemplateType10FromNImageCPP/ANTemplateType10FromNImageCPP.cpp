...
{
	cout << "usage:" << endl;
	cout << "\t" << title << " [NImage] [ANTemplate] [Tot] [Dai] [Ori] [Tcn] [Src] [Encoding]" << endl << endl;
...
int main(int argc, NChar ** argv)
{
	...
	try
	{
		...
		*/
		NBuffer imageData = NFile::ReadAllBytes(argv[1]);
		...
		if (imt == anitFace)
		{
			ANDistortion distortion = ANDistortion(andcBarrel, andmcEstimated, andscMild);
			record.SetDistortion(&distortion);
			record.GetLightingArtifacts().Add(anlaFaceShadows);
			record.SetSubjectPose(anspFullFaceFrontal);
			record.SetTieredMarkupCollection(antmcEyeCenters);
			record.SetImageTransformation((BdifFacePostAcquisitionProcessing)(bfpapPoseCorrected | bfpapContrastStretched));
		}
