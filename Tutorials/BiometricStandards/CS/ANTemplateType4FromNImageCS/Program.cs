...
namespace Neurotec.Tutorials
{
	class Program
	{
		private static int Usage()
		{
			...
			Console.WriteLine("\t{0} [NImage] [ANTemplate] [Tot] [Dai] [Ori] [Tcn] [Encoding]", TutorialUtils.GetAssemblyName());
			Console.WriteLine("");
		...
		static int Main(string[] args)
		{
			...
			try
			{
				...
				uint imageScanningResolution = ANType1Record.MinScanningResolution;// image scanning resolution
				float imageResolutionPpi = 500;// image current resolution
				...
				*/
				...
				using (var imageBuffer = NFile.ReadAllBytes(args[0]))
				{
					...
					ANType4Record record = template.Records.AddType4(true, imageBuffer);
