...
import com.neurotec.lang.NeurotecException;
...
public final class Utils {
	...
	public static final String VERSION = "13.1.0.0";
	public static final String COPYRIGHT = "Copyright © 2011-2024 Neurotechnology";

	private static final boolean DEFAULT_TRIAL_MODE = false;

	public static boolean getTrialModeFlag() throws IOException {
		Path p = Paths.get("..", "Licenses", "TrialFlag.txt");
		if (p.toFile().exists()) {
	...
	public static int printException(Throwable ex) {
		int errorCode = -1;
		NeurotecException neurotecException = null;
		if (ex instanceof AggregateExecutionException) {
			System.out.println("Execution resulted in one or more errors:\n");
			for (Throwable cause : ((AggregateExecutionException) ex).getCauses()) {
				System.out.println(cause.toString());
				neurotecException = ((NeurotecException) ex);
			}
		} else {
			...
			if(ex.getMessage() != null) {
				...
				neurotecException = ((NeurotecException) ex.getCause());
				System.out.println(ex.getCause().getMessage());
			}
		}

		if (neurotecException != null) {
			errorCode = neurotecException.getCode();
		}
		return errorCode;
	}
