...
public:
	...
	static NImage RotatedCopy(const NImage & image, NInt centerX, NInt centerY, NDouble angle, NInt radiusX, NInt radiusY, NBool interpolateBackground, const NRgb & backgroundColor,
		NUInt resultWidth, NUInt resultHeight, NSizeType resultStride = 0)
	{
		HNImage hResultImage;
		NCheck(NrgbipRotatedCopy(image.GetHandle(), centerX, centerY, angle, radiusX, radiusY, interpolateBackground, backgroundColor, resultWidth, resultHeight, resultStride, &hResultImage));
		return NObject::FromHandle<NImage>(hResultImage);
	}
};
