...
public:
	...
	static void Add(const IO::NBuffer & licenseBuffer)
	{
		NCheck(NLicenseAddFromBuffer(licenseBuffer.GetHandle()));
	}
	...
	static NLicenseObtainedStatus GetObtainedStatus(const NStringWrapper & address, const NStringWrapper & port, const NStringWrapper & product)
	{
		NLicenseObtainedStatus value;
		NCheck(NLicenseGetObtainedStatusN(address.GetHandle(), port.GetHandle(), product.GetHandle(), &value));
		return value;
	}

	static NLicenseObtainedStatus GetObtainedComponentStatus(const NStringWrapper & address, const NStringWrapper & port, const NStringWrapper & component)
	{
		NLicenseObtainedStatus value;
		NCheck(NLicenseGetObtainedComponentStatusN(address.GetHandle(), port.GetHandle(), component.GetHandle(), &value));
		return value;
	}
