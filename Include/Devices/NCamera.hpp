...
public:
	...
	::Neurotec::Media::NVideoSample GetVideoSample()
	{
		HNVideoSample hSample = NULL;
		NCheck(NCameraGetVideoSample(GetHandle(), &hSample));
		return FromHandle< ::Neurotec::Media::NVideoSample>(hSample);
	}

	bool IsVideoSampleSupported() const
	{
		NBool value;
		NCheck(NCameraIsVideoSampleSupported(GetHandle(), &value));
		return value != 0;
	}
