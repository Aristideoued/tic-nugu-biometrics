...
public:
	...
	static ::Neurotec::IO::NBuffer MergeBuffer(const NArrayWrapper< ::Neurotec::IO::NBuffer>& buffers, NUInt flags = 0)
	{
		HNBuffer handle;
		NCheck(NTemplateMergeBuffer(const_cast<HNBuffer *>(buffers.GetPtr()), buffers.GetCount(), flags, &handle));
		return FromHandle< ::Neurotec::IO::NBuffer>(handle);
	}

	template<typename InputIt>
	static ::Neurotec::IO::NBuffer MergeBuffer(InputIt first, InputIt last, NUInt flags = 0)
	{
		NArrayWrapper< ::Neurotec::IO::NBuffer> buffers(first, last);
		HNBuffer handle;
		NCheck(NTemplateMergeBuffer(const_cast<HNBuffer *>(buffers.GetPtr()), buffers.GetCount(), flags, &handle));
