...
public:
	...
	::Neurotec::IO::NBuffer Save(BdifEncodingType encodingType = betTraditional, NUInt flags = 0) const
	{
		HNBuffer hBuffer;
		NCheck(FIRecordSaveToMemoryN(GetHandle(), encodingType, flags, &hBuffer));
		return FromHandle< ::Neurotec::IO::NBuffer>(hBuffer);
	}

	void Save(::Neurotec::IO::NStream & stream, BdifEncodingType encodingType = betTraditional, NUInt flags = 0) const
	{
		NCheck(FIRecordSaveToStream(GetHandle(), stream.GetHandle(), encodingType, flags));
	}
