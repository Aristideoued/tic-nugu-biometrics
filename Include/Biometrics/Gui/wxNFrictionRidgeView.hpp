...
public:
	...
	wxNFrictionRidgeView(wxWindow *parent, wxWindowID winid = wxID_ANY)
		: wxNView(parent, winid), m_minColor(0, 230, 0), m_maxColor(255, 255, 255), m_record(NULL), m_showPatternClassConfidence(false), m_showWrongHandWarning(true), m_showTipsWarning(true), m_showFingerAngleWarning(true), m_qualityStyle(STARS), m_boundingRectColor(wxColor(255, 0, 0)), m_frictionRidge(NULL)
	{
	...
	bool GetShowTipsWarning()
	{
		return m_showTipsWarning;
	}

	void SetShowTipsWarning(bool value)
	{
		if (value != m_showTipsWarning)
		{
			m_showTipsWarning = value;
			Refresh();
		}
	}

	bool GetShowFingerAngleWarning()
	{
		return m_showFingerAngleWarning;
	}

	void SetShowFingerAngleWarning(bool value)
	{
		if (value != m_showFingerAngleWarning)
		{
			m_showFingerAngleWarning = value;
			Refresh();
		}
	}
...
private:
	...
	void DrawQualityAndPatternClass(wxGraphicsContext * gc, const ::Neurotec::Biometrics::NFAttributes & attributes, const ::Neurotec::Geometry::NRect & rect)
	{
		...
		::Neurotec::Biometrics::NfiqQuality quality = NBiometricTypes::Nfiq1ToNfiqQuality(attributes[nbaiNfiq10].GetValue());
	...
	virtual void OnDraw(wxGraphicsContext *gc)
	{
		...
		if (!m_frictionRidge.IsNull())
		{
			...
			if (m_showTipsWarning && NFinger::NativeTypeOf().IsInstanceOfType(m_frictionRidge))
			{
				NFinger currentFinger = NObjectDynamicCast<NFinger>(m_frictionRidge);
				if (currentFinger.GetTipsWarning())
				{
					wxSize sz = GetImageRect().GetSize();
					float scale = 800.0f / sz.GetWidth();
					wxFont font(wxFontInfo(16 * scale).FaceName("Arial"));
					gc->SetFont(font, wxColor(250, 190, 0));
					gc->DrawText("Warning: Possibly fingers tips", 10, 26);
				}
			}
			if (m_showFingerAngleWarning && NFinger::NativeTypeOf().IsInstanceOfType(m_frictionRidge))
			{
				NFinger currentFinger = NObjectDynamicCast<NFinger>(m_frictionRidge);
				if (currentFinger.GetFingerAngleWarning())
				{
					wxSize sz = GetImageRect().GetSize();
					float scale = 800.0f / sz.GetWidth();
					wxFont font(wxFontInfo(16 * scale).FaceName("Arial"));
					gc->SetFont(font, wxColor(250, 190, 0));
					gc->DrawText("Warning: Possibly wrong angle of finger", 10, 41);
				}
			}
	...
	bool m_showTipsWarning;
	bool m_showFingerAngleWarning;
