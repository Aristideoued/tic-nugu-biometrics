...
public:
	...
	NByte GetNfiq(NVersion version) const
	{
		NByte value;
		NCheck(NFRecordGetNfiq((HNFRecord)GetHandle(), version.GetValue(), &value));
		return value;
	}

	void SetNfiq(NVersion version, NByte value)
	{
		NCheck(NFRecordSetNfiq((HNFRecord)GetHandle(), version.GetValue(), value));
	}
