...
public:
	...
	::Neurotec::Images::NImage GetRidgeAreaImage() const
	{
		HNImage hValue;
		NCheck(NFrictionRidgeGetRidgeAreaImage(GetHandle(), &hValue));
		return FromHandle< ::Neurotec::Images::NImage>(hValue, true);
	}

	void SetRidgeAreaImage(const ::Neurotec::Images::NImage & value) const
	{
		NCheck(NFrictionRidgeSetRidgeAreaImage(GetHandle(), value.GetHandle()));
	}
