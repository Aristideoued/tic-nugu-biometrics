...
public:
	...
	class SignatureCollection : public ::Neurotec::Collections::NCollectionWithChangeNotifications<NSignature, NSubject,
		NSubjectGetSignatureCount, NSubjectGetSignature, NSubjectGetSignatures,
		NSubjectAddSignaturesCollectionChanged, NSubjectRemoveSignaturesCollectionChanged>
	{
		SignatureCollection(const NSubject & owner)
		{
			SetOwner(owner);
		}
	...
	public:
		...
		{
			NInt result;
			NCheck(NSubjectGetSignatureCapacity(this->GetOwnerHandle(), &result));
			return result;
		}

		void SetCapacity(NInt value)
		{
			NCheck(NSubjectSetSignatureCapacity(this->GetOwnerHandle(), value));
		}

		void Set(NInt index, const NSignature & value)
		{
			NCheck(NSubjectSetSignature(this->GetOwnerHandle(), index, value.GetHandle()));
		}

		NInt Add(const NSignature & value)
		{
			NInt result;
			NCheck(NSubjectAddSignature(this->GetOwnerHandle(), value.GetHandle(), &result));
			return result;
		}

		void Insert(NInt index, const NSignature & value)
		{
			NCheck(NSubjectInsertSignature(this->GetOwnerHandle(), index, value.GetHandle()));
		}

		void RemoveAt(NInt index)
		{
			NCheck(NSubjectRemoveSignatureAt(this->GetOwnerHandle(), index));
		}

		void Clear()
		{
			NCheck(NSubjectClearSignatures(this->GetOwnerHandle()));
		}
...
public:
	...
	SignatureCollection GetSignatures()
	{
		return SignatureCollection(*this);
	}

	const SignatureCollection GetSignatures() const
	{
		return SignatureCollection(*this);
	}
