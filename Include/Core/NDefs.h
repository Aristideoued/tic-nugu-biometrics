...
#define N_STATIC_ASSERT_EX(x) sizeof(char[1 - 2 * !(x)])
#define N_STATIC_ASSERT(x) ((void)N_STATIC_ASSERT_EX(x))
	...
	#ifdef _MM_HINT_T0
		#define N_PREFETCH(x) _mm_prefetch((const char *)(x), _MM_HINT_T0)
	#else
		#define N_PREFETCH(x) (void)(x)
	#endif
#elif defined(N_GCC)
...
#define N_STRINGIZE2(x) #x
#if defined(N_CLANG) && N_CLANG_VERSION >= 30500
	#define N_UNROLL(n_) _Pragma(N_STRINGIZE2(unroll n_))
#elif defined(N_GCC) && N_GCC_VERSION >= 80000
	#define N_UNROLL(n_) _Pragma(N_STRINGIZE2(GCC unroll n_))
#else
	#define N_UNROLL(n_)
#endif
...
#if defined(__SSE_MATH__) || (defined(_M_IX86_FP) && _M_IX86_FP > 0) || defined(N_X64)
	#define N_X86_SSE_MATH
#endif
...
#if defined(N_ANDROID) || defined(N_IOS) || defined(N_ARM_FAMILY)
	#define N_EMBEDDED
#endif
