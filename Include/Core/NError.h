...
#define N_ERROR_CPP_M_A(function, szMessage, flags) \
	{\
		N_THE_ERROR = NErrorSetLastA(N_E_CPP, szMessage, NULL, 0, N_ERROR_MAKE_EXTERNALL_CALL_STACKA(function), flags);\
		N_ERROR_APPEND\
		goto N_TRY_End;\
	}
...
#if (defined(N_DEBUG) || !defined(NDEBUG)) && !defined(N_NO_ANSI_FUNC)
#ifndef N_TRACE_H_INCLUDED
NResult N_API NDebugLogA(NInt level, const NAChar * szCategory, const NAChar * szFormat, ...);
#define N_DEBUG_LOGA_DEFINED 1
#endif
#define N_ERROR_CHECK_ERROR_IN_FINALLY() \
       if (N_WAS_ERROR) NDebugLogA(2, "N_FINALLY", "{S}:{I} failure {I} in N_FINALLY block", __FILE__, __LINE__, N_THE_ERROR)
#else
#define N_ERROR_CHECK_ERROR_IN_FINALLY()
#endif

#define N_FINALLY \
	N_SOMETIMES_UNUSED_VARIABLE(N_THE_ERROR);\
	N_TRY_End:\
	if (N_UNLIKELY(N_FINALLY_entered))\
	{\
		N_ERROR_CHECK_ERROR_IN_FINALLY();\
	}\
	else { N_FINALLY_entered = NTrue; {
